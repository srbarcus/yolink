 ...
 * 
 *  2.0.0: Initial Release. Note: CellularHub defined in API specs but called as "Hub".
 */
...
preferences {
    input title: bold("Driver Version"), description: "YoLink™ Cellular Hub (YS1605-UC) v${clientVersion()}${copyright()}", displayDuringSetup: false, type: "paragraph", element: "paragraph"
...
metadata {
    definition (name: "YoLink CellularHub Device", namespace: "srbarcus", author: "Steven Barcus", singleThreaded: true) {     		
		capability "Polling"
...
def getDevicestate() {
	...
	try {  
        def request = [:]
            request.put("method", "Hub.getState")                
...
def parseDevice(object) {
    //Plug in
    //data:[cellular:[apn:hologram.mnc001.mcc222.gprs, enable:true, gateway:192.168.0.255, iccid:8935711220003921221, ip:192.168.0.100, mask:255.255.255.0, 
    //  module:[enable:true, imei:867284061962397, mfr:SIMCOM INCORPORATED, model:A7670G-LABE, ver:A7670M7_V1.11.1],
    //regState:roaming, regStateCode:5, rssi:-85], 
    //eth:[enable:true, gateway:192.168.1.255, ip:192.168.1.35, mask:255.255.255.0], 
    //online:true, other:[power:[battery:true, batteryState:2, dc:true], supportNRP:true], version:0506, 
    //wifi:[enable:true, gateway:192.168.1.255, ip:192.168.1.13, mask:255.255.255.0, ssid:BAP24]]    
    
    //Un Pluged
    //data:[cellular:[apn:hologram.mnc001.mcc222.gprs, enable:true, gateway:192.168.0.255, iccid:8935711220003921221, ip:192.168.0.100, mask:255.255.255.0,
    //  module:[enable:true, imei:867284061962397, mfr:SIMCOM INCORPORATED, model:A7670G-LABE, ver:A7670M7_V1.11.1], 
    //regState:roaming, regStateCode:5, rssi:-87], 
    //eth:[enable:true, gateway:192.168.1.255, ip:192.168.1.35, mask:255.255.255.0], 
    //online:true, other:[power:[battery:true, batteryState:1, dc:false], supportNRP:true], version:0506, 
    //wifi:[enable:true, gateway:192.168.1.255, ip:192.168.1.13, mask:255.255.255.0, ssid:BAP24]],
    ...
    def statendx = object.data.other.power.batteryState
    def rssi = object.data.cellular.rssi  
    
    batteryState = batteryStates["${statendx}"]
    ...
    logDebug("Hub Firmware Version (${firmware}), " +
     ...
     rememberState("batteryState", batteryState)
     fmtSignal(rssi) 
}                

void resetCellular() { 
   logDebug "Resetting Cellular"
      
   def request = [:]
   request.put("method", "Hub.resetCellular")                
   request.put("targetDevice", "${state.devId}") 
   request.put("token", "${state.token}") 
   ...
   if (object) {
       logDebug("resetCellular(): pollAPI() response: ${object}")                          
...
@Field static final Map batteryStates = [
  "0":"No battery connected",
  "1":"Battery is powering the hub",
  "2":"Battery is charging",
  "3":"Battery is standing by",
  "4":"Battery under maintenance cycle"
  ]
