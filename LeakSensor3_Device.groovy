...
// Note: Sensitivity cannot be change in mobile app at this time
...
metadata {
    definition (name: "YoLink LeakSensor3 Device", namespace: "srbarcus", author: "Steven Barcus", singleThreaded: true) {     	
		capability "Polling"	
...
def void processStateData(payload) {
    ...
    if (state.devId == devId) {  // Only handle if message is for me 
		...
		case "setInterval":
            ...
            logDebug("Parsed: Interval=$interval, Beep=$beep, Mode=$mode, RSSI=$rssi")  
...
def waterMode(value) {
   switch(value) {
		case "WaterPeak":
             return "WaterExhausted"
        case "WaterLeak":                  
             return value            
		default:
            log.error "Unknown water mode received: $value"
            return value 
			break;
	    }  
  }    
