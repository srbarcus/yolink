 ...
 *  2.0.13: Add option to ignore garage state for open and close
 *  2.0.14: Added  showInterimState command to suppress "opening" and "closing"
 */

import groovy.json.JsonSlurper

def clientVersion() {return "2.0.14"}
...
def toggle(func) {
   ...
   logDebug("toggle(${func}): Door $currentState, IgnoreDoorState = $ignoreDoor, interimState = $interimState") 
   if (!boundToDevice()) { 
